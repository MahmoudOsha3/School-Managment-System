...
@endsection
@section('page-header')
    <h3>{{trans('student.edit_question')}}</h3>
    <br>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text"  name="right_answer" value="{{ $question->right_answer }}" class="form-control" >
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="custom-select mr-sm-2" name="score">
                                        <option disabled>{{trans('student.Choose')}}...</option>
                                        @for ($i = 1 ; $i <= 10 ; $i++)
                                            <option value="{{ $i }}" @selected($question->score == $i)>{{ $i }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="custom-select mr-sm-2" name="quizze_id">
                                        ...
                                        @foreach($quizzes as $quizze)
                                            <option value="{{ $quizze->id }}" @selected($quizze->id == $question->quizze->id )>{{ $quizze->grade->name }} - {{ $quizze->classroom->name }} - {{ $quizze->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
