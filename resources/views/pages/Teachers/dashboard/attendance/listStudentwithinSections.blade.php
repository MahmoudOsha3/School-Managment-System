                                            ...
                                            <td>
                                                ...
                                                <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
                                                    <input name="attendences[{{ $student->id }}]" class="leading-tight" type="radio"
                                                        @foreach($student->attendance()->where('date_attednace' , date('Y-m-d'))->get() as $attendance )
                                                            {{ $attendance->attednace_status == 1 ? 'checked' : '' }}
                                                        @endforeach
                                                        value="{{ true }}">
                                                ...
                                                <label class="ml-4 block text-gray-500 font-semibold">
                                                    <input name="attendences[{{ $student->id }}]" class="leading-tight" type="radio"
                                                    @foreach($student->attendance()->where('date_attednace' , date('Y-m-d'))->get() as $attendance )
                                                        {{ $attendance->attednace_status == 0 ? 'checked' : '' }}
                                                    @endforeach
                                                        value="{{ 0 }}">
